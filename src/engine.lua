...
function engine.key(event: any): any
    ...
    local map: any = {["."] = "dot", [","] = "dot", ["+"] = "add", ["-"] = "sub",
        ["*"] = "mul", ["/"] = "div", ["="] = "eq", ["%"] = "pct",
        ["r"] = "inv", ["@"] = "sqrt", ["c"] = "c", ["C"] = "c"}
    -- Cyrillic es, lower and upper (U+0441, U+0421): the same physical key as
    -- C on a Russian layout, so Clear works without switching layouts. Written
    -- as byte escapes: the source carries no non-ASCII literal.
    map["\209\129"] = "c"
    map["\208\161"] = "c"
    return map[key]
end
