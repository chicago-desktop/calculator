...
- **`make test` runs the harness in `test/`** with `--host
  wippy.terminal:host`; it writes `test/shots/calculator.png`, the window as
  the shell's renderer drew it after `12 × 3.5 =`. Look at the picture: the
  geometry checks do not see a wrong colour or a caption a pixel off.
- **The engine is pure and the window only draws it.** `src/engine.lua`
  counts like a desk calculator (`2 + 3 × 4 =` is 20) and knows no screen;
  a change to how it counts gets a case in `test/src/engine_test.lua` first.
...
- **`wippy publish` packs only `src/`**; the calculator has no image pack of
  its own (its icon is the shell's), so `wippy.yaml` has no `embed:` list.
