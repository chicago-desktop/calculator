...
The Calculator, a module of the Chicago shell for the terminal
...
- **The arithmetic is pure, the window is data.** Everything about how the
  calculator counts lives in `src/engine.lua`, a library with no screen and
  no runtime; `src/window.lua` is the key grid as a component tree, run with
  `app.main`. A rule of the calculator goes into the engine and gets a case
  in `test/src/engine_test.lua`, never into the window. Use the SDK as documented in `docs/sdk.md` and the
...
make test           # the harness's suites; writes test/shots/calculator.png
...
- **`wippy publish` packs only `src/`** and embeds only the `fs.directory`
  entries `wippy.yaml` lists under `embed:`. The calculator has none — its
  icon is the shell's `calculator` — so a picture of its own would need a
  pack and an `embed:` line; `make check` verifies it.
